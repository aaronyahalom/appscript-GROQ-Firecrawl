...
const PROPERTY_KEY_OPENAI = "OPENAI_API_KEY" as const;
const PROPERTY_KEY_OPENROUTER = "OPENROUTER_API_KEY" as const;
 ...
 */
function REQUEST_COMPLETIONS_UNIFIED(
 ...
 */
function TEST_OPENAI_CONNECTION(apiKey: string) {
  try {
    const response = REQUEST_COMPLETIONS_UNIFIED(
      'openai',
      apiKey,
      '',
      'Test connection',
 ...
 */
function TEST_OPENROUTER_CONNECTION(apiKey: string) {
  try {
    const response = REQUEST_COMPLETIONS_UNIFIED(
      'openrouter',
      apiKey,
...
function GET_OPENROUTER_MODELS() {
  ...
  return data.data.map((model: any) => ({
    ...
    if (a.top_provider && !b.top_provider) return -1;
    if (!a.top_provider && b.top_provider) return 1;
 ...
 */
function SAVE_OPENROUTER_SETTINGS(settings: { apiKey: string; defaultModel: string }) {
  const properties = PropertiesService.getUserProperties();
 ...
 * @customfunction
 */
function OPENAI(prompt: string, model?: string, temperature?: number, maxTokens?: number): string {
  const properties = PropertiesService.getUserProperties();
  ...
  return REQUEST_COMPLETIONS_UNIFIED(
    'openai',
    apiKey,
    systemPrompt,
    prompt,
 ...
 * @customfunction
 */
function OPENROUTER(prompt: string, model?: string, temperature?: number, maxTokens?: number): string {
  const properties = PropertiesService.getUserProperties();
